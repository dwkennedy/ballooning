...
/**************************************************************************/
/*!
    @brief Read and calculate the pressure in digital units
    @returns The measured pressure, in Pa on success, NAN on failure

*/
/**************************************************************************/
uint32_t Adafruit_MPRLS::readIntPressure(void) {
  const uint32_t _Pa_max = 172369L;
...
//  Pa /= (float)(_OUTPUT_max - _OUTPUT_min);
//  Pa += _Pa_min;

/*  24 bit * 18 bit  --> 18 bit * 14 bit = 32 bit using shift rights
 *   divided by 18 bit gives 14 bits
 *   shifted 4 left, 18 bits
 */
